...
def NUS_WIDE_load_two_party_data(data_dir, selected_labels, neg_label=-1, n_samples=-1):
    ...
    train_Xa, train_Xb, y_train = get_labeled_data_with_2_party(data_dir=data_dir,
                                              selected_labels=selected_labels,
                                              n_samples=n_samples)

    test_Xa, test_Xb, y_test = get_labeled_data_with_2_party(data_dir=data_dir,
                                                                selected_labels=selected_labels,
                                                                n_samples=n_samples, dtype="Test")
    y_train = torch.from_numpy(y_train)
    y_test = torch.from_numpy(y_test)
    y_train = torch.argmax(y_train, dim=1)
    y_test = torch.argmax(y_test, dim=1)

    print("Train_Xa shape:", train_Xa.shape)
    print("Train_Xb shape:", train_Xb.shape)
    print("Train y shape:", y_train.shape)
    print("Test_Xa shape:", test_Xa.shape)
    print("Test_Xb shape:", test_Xb.shape)
    print("Test y shape:", y_test.shape)


    return [train_Xa, train_Xb, y_train], [test_Xa, test_Xb, y_test]
